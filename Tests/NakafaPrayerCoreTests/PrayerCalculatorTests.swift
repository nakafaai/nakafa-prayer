...
struct PrayerCalculatorTests {
    ...
    func fixedPrayerTimesMatchAdhanLibraryReference() throws {
        ...
        #expect(normalized(formatter.string(from: time(.fajr, in: schedule))) == "5:35 AM")
        #expect(normalized(formatter.string(from: time(.dhuhr, in: schedule))) == "12:05 PM")
        #expect(normalized(formatter.string(from: time(.asr, in: schedule))) == "2:42 PM")
        #expect(normalized(formatter.string(from: time(.maghrib, in: schedule))) == "5:01 PM")
        #expect(normalized(formatter.string(from: time(.isha, in: schedule))) == "6:26 PM")
    }
